...
# There is an inconsistency in the way that wire edge extrusions are handled.  On the mesh boundary the walker actively checks for them and considers them a dead end (unless the ignore preference is on).
#     In the manifold interior of the topology, however, there is no check for wire extrusions at all which means the manifold vertex and edge walkers will go straight through them.
#     I shudder to think of the performance impact of checking for them at every step but it's technically not such a good thing to treat them inconsistently.
# Extension methods:
#     [X] manifold vertex,
#         [X] Full loop
#         [X] Bounded loop
...
#    ("SUB", "Subtract", "Subtract from the existing selection", 3),
    ...
    def execute(self, context):
        ...
        if context.object.mode == ObjectMode.EDIT:
            ...
            if context.tool_settings.mesh_select_mode[0]: # It's a tuple; I could test if mesh_select_mode == (1,0,0)?
    ...
    if not previous_active_vert.index == active_vert.index:
        ...
        if adjacent:
            ...
            if active_edge.is_manifold:
                ...
                print("Selecting Wire Vert Loop")
...
#                    print("entire_loop runtime: %.20f sec" % (t1 - t0))  # Delete me later
                    ...
                    if len(new_sel) == 1:  # I'm not sure if this condition is ever true due to filters elsewhere
                        new_sel = None
...
def get_bounded_selection(component0, component1, mode):
    ...
    if mode == 'VERT':
        # Not implemented yet but for manifold interior selections if the len(v.link_edges) for one of the verts is 3, 
        # and the other is 4 we could maybe use the n=3 vert as the starting vert.
        # Official Blender definitions for non-manifold vertex.
...
#            elif c0.is_boundary and len(c0_wire) > 0 and not c1.is_manifold and len(c1_wire) == 0:  # One boundary wire extrusion and one internal non-manifold edge extrusion
                # non-functional because I haven't pioneered doing bounded_loop_vert_manifold starting at a boundary; it would need to know that the number of proper manifold edges connected to that vert is exactly 1 (or at least <= 4)
            print("Starting vert:", starting_vert.index)
        ...
        else:  # Any other condition that I've missed
            return None
    ...
    if mode == 'EDGE':
        ...
        if c0.is_manifold and c1.is_manifold:  # Manifold
            ...
            if len(c0_loop_dirs):
                ...
                connected_loops = bounded_loop_edge_manifold(prefs, starting_edge, ends)  # Slightly faster
                e1 = time.perf_counter()
                print("bounded_loop_edge_manifold runtime: %.20f sec" % (e1 - e0))  # Delete me later
            if len(connected_loops) > 0:
                ...
                print("bounded_ring_edge_manifold runtime: %.20f sec" % (e3 - e2))  # Delete me later
...
#    print([len(r) for r in connected_loops])
    ...
    # Possible future upgrade: Return the loop whose end is closer to the mouse cursor.
        ...
        return {i for loop in connected_loops if len(loop) == len(connected_loops[0]) for i in loop}  # Maybe instead we could just merge the sets together rather than building a new set that contains i from all N sets and see which performs better.
...
#                if len(candidate_dirs) == 4:  # Future note to self about possible use of valence-3 vert as starting_vert (no opposite_edge)
    ...
    print("bounded_loop_vert_manifold runtime: %.20f sec" % (endtime - begintime))  # Delete me later
...
def bounded_loop_face(prefs, starting_face, ends):
    ...
    endtime = time.perf_counter()
    print("bounded_loop_face runtime: %.20f sec" % (endtime - begintime))  # Delete me later
    return connected_loops
...
#    next_vert = [v for v in cur_edge.verts if v is not cur_vert][0]  # Which if these is faster?
    partial_list = {cur_vert}

    while True:
        partial_list.add(next_vert)
        linked_edges = next_vert.link_edges
#        linked_edges = [e for e in next_vert.link_edges if not e.hide]
        if len(linked_edges) < 2:
            break
...
#        next_vert = [v for v in next_edge.verts if v is not cur_vert][0]  # Which if these is faster?
        cur_edge = next_edge
    return partial_list
...
#    next_vert = [v for v in cur_edge.verts if v is not cur_vert][0]  # Which if these is faster?
    ...
    while True:
#        print("Adding", cur_edge.index)  # Delete me later
#        partial_list.add(cur_edge)
        linked_edges = next_vert.link_edges
        if len(linked_edges) < 2:
            break
        next_edge = [e for e in next_vert.link_edges if e is not cur_edge][0]
...
#        face.tag = False
    ...
    is_hidden = not prefs.ignore_hidden_geometry and (face.hide or edge.hide)  # Note: Can't hide an edge connected to a face in Blender without the face also being hidden so this might be redundant.
#    # Triangle or n-gon
